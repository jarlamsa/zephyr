...
static bool has_ongoing_requests()
{
				    ...
				    bool reconstruct)
{
	...
	internal_req->request_tkl = coap_header_get_token(&internal_req->request,
							  internal_req->request_token);
out:
	return ret;
}
...
static int coap_client_resend_handler()
{
	int ret = 0;
...
static int handle_poll()
{
	...
	while (1) {
		struct pollfd fds[CONFIG_COAP_CLIENT_MAX_INSTANCES] = {0};
		int nfds = 0;
		...
		ret = poll(fds, nfds, COAP_PERIODIC_TIMEOUT);
...
struct coap_client_internal_request *get_request_with_id(struct coap_client *client,
							 uint16_t message_id)
{
	LOG_INF("Received message ID: %d", message_id);
	for (int i = 0; i < CONFIG_COAP_CLIENT_MAX_REQUESTS; i++) {
		LOG_INF("Stored message ID: %d", client->requests[i].pending.id);
		if (client->requests[i].request_ongoing == true &&
