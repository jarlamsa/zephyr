...
ZTEST_SUITE(coap_client, NULL, NULL, setup, NULL, NULL);
...
ZTEST(coap_client, test_get_request)
{
	...
	ret = coap_client_req(0, &address, &client_request, -1);
...
ZTEST(coap_client, test_unmatching_tokens)
{
	...
	LOG_INF("Send request");
	ret = coap_client_req(0, &address, &client_request, 0);
	zassert_true(ret >= 0, "Sending request failed, %d", ret);
	set_socket_events(ZSOCK_POLLIN);
	...
	k_sleep(K_MSEC(1));
	clear_socket_events();
	zassert_equal(last_response_code, COAP_RESPONSE_CODE_NOT_FOUND, "Unexpected response %d", last_response_code);
	k_sleep(K_MSEC(1));
}
